...
class Visit extends Component {
  ...
  open = e => {
    ...
    this._interval = setInterval(this.getVisitList, 1000)
  }
    ...
    return visitList.map((contact, i) => {
      return (
        <Card
          ...
          key={i}
          border={this.state.cardver[i % 10]}
          style={{ width: "100%" }}
        >
          <Card.Body>
            <Card.Title>{contact.name}</Card.Title>
            <Card.Text>{contact.contents}</Card.Text>
          </Card.Body>
        </Card>
      )
    })
              ...
              <div className="row">
                <div className="col-md-5" align="center">
                  ...
                  <QRCode
                    ...
                    value={`http://${process.env.REACT_APP_IP}/visitPage/${this.props.eventInfo.id}`}
                    size={256}
                  />
                ...
                <div className="col-md-7">
                  {/* 방명록자리 */}
                  <CardColumns>{this.showVisits(this.state.visitList)}</CardColumns>
                </div>
              </div>
            </div>
          </Modal.Body>
