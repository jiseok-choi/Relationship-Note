...
router.post("/getNewsList", isLoggedIn, async (req, res, next) => {
  try {
    ...
    result.friend = await Friend.findOne({ where: { userid: req.user.id, id: friendid } })
    result.newsList = await News.findAll({
      where: { userid: req.user.id, friendid: friendid },
      order: [["createdAt", "DESC"]],
      raw: true
    })
    await result.newsList.map((prop, key) => (prop.index = key + 1))
